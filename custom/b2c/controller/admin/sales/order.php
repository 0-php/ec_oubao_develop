...
class b2c_ctl_admin_sales_order extends desktop_controller{
     ...
     * 添加新规则
     */
    //by zhangyan update 2015-8-24 添加新规则，能够指定会员
    public function add() {
    ...
    public function edit($rule_id) {
        ...
        //by zhangyan 2015-08-28 如果指定了会员，找出指定的会员
        if($aRule['appoint'] == 'true'){
            $appoint = $this->app->model("sales_appoint")->getList('member_id',array('sales_id'=>$aRule['rule_id'],'sales_type'=>'orders'));
            $memberIdTmp = array();
            if(!empty($appoint))
    ...
    public function toAdd() {
        ...
        {
            $sales_id= $mSRO->getRow('rule_id',array('name'=>trim($_POST['rule']['name'])),0,3,'rule_id DESC');
            //组参数
            $appointArr=array(
            ...
            $appoint = $this->app->model("sales_appoint");
            $res = $appoint->save_appoint($appointArr);
        }
