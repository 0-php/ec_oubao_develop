...
class b2c_ctl_admin_sales_goods extends desktop_controller{
    ...
    */
    public function add() {
        ...
        //添加是否打开窗口的标记，默认为不打开
        $this->pagedata['rule']['appoint']='false';
        $this->_editor();
    }
    ...
    public function edit($rule_id) {
        ...
        //by zhangyan 2015-08-25 如果指定了会员，把指定会员列出来
        if($aRule['appoint'] == 'true'){
            $appoint_member_ids = $this->app->model("sales_appoint")->getList('member_id',array('sales_id'=>$aRule['rule_id'],'sales_type'=>'goods'));
            $memberIdTmp = array();
            if(!empty($appoint_member_ids))
                foreach($appoint_member_ids as $v)
                {
                    $memberIdTmp[]=$v['member_id'];
                }
            $this->pagedata['rule']['member_id'] = $memberIdTmp;
        }
    ...
    public function toAdd() {
        ...
        //添加指定会员
        if($_POST['rule']['appoint'] === 'true' && empty($_POST['member']['member_id']) === TRUE)
        {
            $this->end( false,'请指定会员！'  );
        }
        ...
        //如果是指定了会员则保存
        if($_POST['rule']['appoint'] === 'true')
        {

            $sales_id= $oGoodsPromotion->getRow('rule_id',array('name'=>trim($_POST['rule']['name'])),0,3,'rule_id DESC');
            //组参数
            $appointArr=array(
                'sales_id'=>$sales_id['rule_id'],
                'sales_type'=>'goods',
                'member_id'=>$_POST['member']['member_id'],
            );
            $appoint_model = $this->app->model("sales_appoint");
            $res = $appoint_model->save_appoint($appointArr);
        }
